...
const Footer = () => {
  ...
  return (
    <footer style={{ background: footerBg, color: textPrimary }}>
      <div style={{ height: "2px", background: "linear-gradient(90deg, var(--gold), var(--steel-light), transparent)" }} />
      <div className="container-wide py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          >
            ...
            <p className="text-sm leading-relaxed mb-6" style={{ color: textLight }}>
              North Region’s  trusted partner for precision facility management and engineering services — for over two decades.
            </p>
            <div className="flex gap-2">
              {[{ icon: "Linkedin", href: "#" }, { icon: "Instagram", href: "#" }, { icon: "Facebook", href: "#" }, { icon: "Youtube", href: "#" }].map((s) => (
                <a key={s.icon} href={s.href} target="_blank" rel="noopener noreferrer"
                  className="w-8 h-8 rounded-lg flex items-center justify-center transition-all hover:bg-[rgba(200,150,62,0.15)]"
                  style={{ background: socialBg, color: dark ? "#8B9BB0" : "#666" }}
                >
          ...
          {/* Navigation */}
          <div>
            <p className="text-[10px] tracking-[0.2em] uppercase mb-5" style={{ color: "var(--gold)", fontFamily: "'DM Mono', monospace" }}>Navigation</p>
            <ul className="space-y-3">
              {[...NAV_LINKS, { id: "enquiry", label: "Get a Quote" }].map((l) => (
                <li key={l.id}>
                  <Link
                    to={l.id === "home" ? "/" : `/${l.id}`}
                    onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
          ...
          {/* Services */}
          <div>
            <p className="text-[10px] tracking-[0.2em] uppercase mb-5" style={{ color: "var(--gold)", fontFamily: "'DM Mono', monospace" }}>Services</p>
  ...
  <div className="space-y-4">
    {[
      ...
      <div key={i} className="flex gap-3">
        ...
        {/* Bottom */}
        <div className="mt-14 pt-6 flex flex-col sm:flex-row justify-between items-center gap-3" style={{ borderTop: borderColor }}>
          <p className="text-[10px] tracking-[0.15em] uppercase" style={{ color: copyrightText, fontFamily: "'DM Mono', monospace" }}>
